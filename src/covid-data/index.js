...
export default function CovidData() {
  ...
  const countryDataList = countryList
    ? countryList?.data
        ?.filter(
          (datum) =>
            !(datum[0].startsWith("iso") || datum[0].startsWith("OWID"))
        )
        .map((datum) => {
          return {
            value: datum[0],
  ...
  const label = duration
    ? countryData
        ?.map((datum) => datum[2])
        .slice(countryData.length - duration, countryData.length)
          ...
          <div>
            <FormControl sx={{ m: 1, minWidth: 120 }}>
              ...
              <Select value={country} label="Country" onChange={handleChange}>
                {countryDataList &&
                  countryDataList.map((datum) => {
                    return (
                      <MenuItem key={datum.value} value={datum.value}>
                        {datum.label}
                      </MenuItem>
                    );
                  })}
